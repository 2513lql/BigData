...
public class ProvAspect {
        ...
        //stone
        @After(value = "com.wbl.aop.AopPointCut.recordProv() &&" + "@annotation(activity)",argNames = "joinPoint,activity")
        public void record(JoinPoint joinPoint,ProvAnnotation activity) throws Exception{
                HttpServletRequest request = ((ServletRequestAttributes) RequestContextHolder.getRequestAttributes()).getRequest();
                switch (activity.value()){
                        case DOWNLOAD:
                                //recordDownloadProv(isLogin(request),request);
                                recordDownload(joinPoint,request);
                                break;
                        case UPLOAD:
                                recordUpload(joinPoint,request);
                                break;
                        case SPLIT:
                                recordSplit(joinPoint,request);
                                break;
                        case AGGREGATION:
                                //recordAggregationProv(joinPoint);
                                break;
        ...
        public void recordDownload(JoinPoint joinPoint,HttpServletRequest request){

        }
        public void recordUpload(JoinPoint joinPoint,HttpServletRequest request){
                Object[] args = joinPoint.getArgs();
                DataInfo dataInfo = (DataInfo) args[0];
                UserInfo userInfo = (UserInfo)request.getSession().getAttribute("userinfo");
//                provService.recordUpload(dataInfo, userInfo);
        }
        public void recordSplit(JoinPoint joinPoint,HttpServletRequest request){
                Object[] args = joinPoint.getArgs();
                String filename = (String) args[0];
                String type= (String) args[1];
                String originalDataName = (String) args[2];
                UserInfo userInfo = (UserInfo)request.getSession().getAttribute("userinfo");
